...
module.exports = async function pages(app) {
  ...
  app.get('/offer', async (req, reply) => reply.type('text/html').send(views.offer(ctx(req))));
  app.get('/privacy', async (req, reply) => reply.type('text/html').send(views.privacy(ctx(req))));
...
`));
  ...
  app.get('/sitemap.xml', async (req, reply) => {
    const urls = ['/', '/pricing', '/docs', '/openapi.json', '/offer', '/privacy', ...views.DOC_LANGS.map((l) => '/docs/' + l), '/tools/html-to-pdf', '/tools/screenshot'];
