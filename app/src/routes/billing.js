...
module.exports = async function billingRoutes(app) {
  ...
  app.post('/checkout', async (req, reply) => {
    ...
    if (!price || plan === 'free') throw err(400, 'Unknown plan');
    if (!stripe) throw err(503, 'Billing is not configured yet (STRIPE_SECRET_KEY missing)');
  ...
  app.post('/portal', async (req, reply) => {
    const user = requireUser(req);
    if (!stripe) throw err(503, 'Billing is not configured yet');
  ...
  app.register(async function (sub) {
    ...
    sub.post('/webhook', async (req, reply) => {
      ...
      try {
        ...
        throw err(400, 'Invalid signature: ' + e.message);
      }
      ...
      applyEvent(db, config, event, app.log);
      return { received: true };
    });
  }, { prefix: '' });
};

function applyEvent(db, config, event, log) {
  ...
  if (event.type === 'customer.subscription.updated' || event.type === 'customer.subscription.deleted') {
    ...
    const plan = active ? (planFromPrice(config, priceId) || obj.metadata?.plan || user.plan) : 'free';
